...
def generate_and_score(model, loader, ctx, temperature, top_k, n_samples=1):
    ...
    for x in bar:
        ...
        for i in range(n_samples):
            with ctx:
                y_pred.append(
                    model.generate(x, max_new_tokens=num_target_tokens, temperature=temperature, top_k=top_k, do_sample=True, attention_mask = torch.ones_like(x), pad_token_id=2))
        ...
        y_pred_dataset.append(y_pred.cpu())
...
def create_dpo_dataset(prompts, generations, scores):
    ...
    return Dataset.from_dict({'chosen': chosen, 'rejected': rejected, 'prompt': prompts})

ref_model = deepcopy(model)
    
    
for i in range(100):
    prompts, generations, scores = generate_and_score(model, test_loader, ctx, temperature, top_k, n_samples=n_samples)
